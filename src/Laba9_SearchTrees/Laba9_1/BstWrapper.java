...
public class BstWrapper<T extends Comparable<T>> {
    ...
    public void insert(T target) {
        ...
        boolean isInserted;
        if (this.root == null) {
            this.root = new BstNode2<>(target);
            isInserted = true;
        } else {
            isInserted = root.insert(target);
        }
        System.out.println(isInserted ? ("Добавлен объект: " + target) : ("Объект " + target) + " Уже присутствует в дереве");
    }
